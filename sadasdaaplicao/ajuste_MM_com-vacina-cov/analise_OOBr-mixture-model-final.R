...
models3 <- lapply(q, function(a) {
  aux <- ggcrqr::bayesGG_mm(
    linear_pred_mu = tempo_hosp_evolucao ~  grupos + saturacao + vacina_cov, 
    linear_pred_alpha = tempo_hosp_evolucao ~ faixa2 + grupos + obesidade + saturacao + vacina_cov,
    data = dados,
    q = a,
    d = "ind_obito",
    iter = 1000,
  ...
  saveRDS(aux, paste("models_mixture_final_q_", a,".rds", sep = ""))
})
